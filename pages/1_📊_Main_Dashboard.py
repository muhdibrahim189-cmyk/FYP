...
# ── Bootstrap ──────────────────────────────────────────────────────────────────
st.set_page_config(page_title="Dashboard · Fiscal Green", page_icon="📊", layout="wide")
init_db()
require_login()
...
with st.sidebar:
    ...
    render_navigation("📊 Main Dashboard")

    st.markdown("### ⚙️ Filters")
    df_all = load_emissions()
    render_data_quality_notice(df_all)
...
with tab1:
    ...
    if not monthly.empty:
        ...
        if len(total_by_month) >= 4:
            ...
            future_vals = lr.predict(pd.DataFrame({"idx": future_idx}))

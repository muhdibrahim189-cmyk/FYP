...
from data.emission_factors import FACILITIES, SCOPE1_SOURCES, SCOPE2_SOURCES, CARBON_TAX_RATE_MYR

st.set_page_config(page_title="Data Centre · CarbonTrack Pro", page_icon="🗄️", layout="wide")
init_db()
require_login()
...
# ── Sidebar ────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("""<div style='padding:0.3rem 0 0.8rem;text-align:center;'>
...
with st.container():
    ...
    with f4:
        facility_filter = st.multiselect("🏭 Facility", FACILITIES, default=FACILITIES, key="dc_fac")
    with f5:
...
stats = [
    (k1, "Records Found",    f"{len(df):,}",          "entries",         "#34d399"),
    (k2, "Total CO₂e",       f"{total_t:,.1f} t",     "tonnes CO₂e",     "#60a5fa"),
    (k3, "Carbon Tax",       f"MYR {tax_myr:,.0f}",   "@ MYR 35/t",      "#f87171"),
    (k1, "Records Filtered",  f"{len(df):,}",          f"of {len(df_raw):,} total", "#1E3A8A"),
    (k2, "Filtered CO₂e",     f"{total_filt_t:,.1f} t",f"{tax_filt_str}",          "#1E3A8A"),
    (k3, "Est. Carbon Tax",   f"{tax_filt_str}",       f"@ MYR {CARBON_TAX_RATE_MYR}/t", "#dc2626"),
    (k4, "Scope 1 Share",    f"{s1_pct:.1f}%",        "direct emissions",          "#d97706"),
    (k5, "Scope 2 Share",    f"{s2_pct:.1f}%",        "indirect electricity",      "#2563EB"),
]
